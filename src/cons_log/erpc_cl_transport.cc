...
void ERPCConsLogTransport::Initialize(const Properties &p) {
    ...
    const int n_th = 0; // std::stoi(p.GetProperty("threadcount", "1"));  // currently conslog is not serving any rpc requests
    ...
    if (pipelined_) {
        ...
        while (run_) {
            cons_log_->store(run_);  // don't start a new threads to run because we need to use the rpc objects in this thread
        }
...
void ERPCConsLogTransport::server_func(erpc::Nexus *nexus, int th_id, const Properties *p) {
    ...
    if (!rpc_)
        rpc_ = new erpc::Rpc<erpc::CTransport>(nexus, static_cast<void *>(&c), CL_SVR_RPCID_OFFSET + th_id, svr_sm_handler, phy_port);  // use range [192, 255] for CL server rpc id
