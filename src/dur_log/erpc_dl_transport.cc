...
void ERPCDurLogTransport::server_func(erpc::Nexus *nexus, int th_id, const Properties *p) {
    ...
    if (!rpc_)
        rpc_ = new erpc::Rpc<erpc::CTransport>(nexus, nullptr, DL_SVR_RPCID_OFFSET + th_id, svr_sm_handler,
                                               phy_port);  // use range [128, 191] for server rpc id
...
void ERPCDurLogTransport::cl_server_func(const Properties *p) {
    const uint8_t phy_port = std::stoi(p->GetProperty("erpc.phy_port", "0"));
    if (!rpc_)
        rpc_ = new erpc::Rpc<erpc::CTransport>(nexus_, nullptr, DL_RSV_RPCID, svr_sm_handler,
                                               phy_port);  // use 191 for reserved server rpc id
    rpc_use_cnt_.fetch_add(1);

    LOG(INFO) << "Durability log ERPC exclusive DL server transport start on reserved thread";

    while (run_) rpc_->run_event_loop(1000);
...
const bool registered = RPCFactory::RegisterRPC("durlog_erpc_svr", []() {
